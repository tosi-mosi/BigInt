...
enum class OperatorPrecedence { none, low, medium, high };

enum class OperatorAssociativity { none, left, right };

// [?] maybe I won't need this
enum class IntegerType { decimal, hexidemical };
...
// [TODO] after creating Calculator -> should test BigInts for memory leaks
//  (Valgrind?)
...
class Calculator {
 public:
    ...
    class Integer {
     public:
        ...
        std::string mStrRepr;  // it will be pointing to the original
                               // expression's string
                               //  update: no it won't
                               //   (cant use string_view, because it is
                               //   necessary for Integer to own )
        BI mIntRepr;
    ...
    class Token {
     ...
     public:
        ...
        Token(TokenType tokenType, Operator* op, std::shared_ptr<Integer> inte)
            : mTokenType{tokenType}, mOp{op}, mInt{inte} {}
    };
 ...
 private:
    static Integer evaluate_postfix() {
        ...
        for (auto it{mTokens.begin()}, end{mTokens.end()}; it != end; ++it) {
            if (it->mTokenType == TokenType::integer) {
                ...
                BI subResult = it->mOp->func(&leftOperand.mInt->mIntRepr,
                                             rightOperand.mInt->mIntRepr);

                operand_stack.emplace(TokenType::integer, nullptr,
        ...
        for (auto it{mTokens.begin()}, end{mTokens.end()}; it != end; ++it) {
            if (it->mTokenType == TokenType::integer) {
                ...
                if (!operator_stack.empty()) {
                    ...
                    // decide whether to pop operators from stack, before
                    // pushing current operator on to it(depends on precedence
                    // and associativity):
                    while (
                        !operator_stack.empty()  // check if stack is not empty
                        &&
                        (top_el =
                             &operator_stack.top())  // read top element(can't
                                                     // be nullptr here)
                        && top_el->mOp->strRepr !=
                               "("  // it shouldn't be left parentheses
                        && (top_el->mOp->prec >
                                it->mOp->prec  // if in stack operator's
                                               // precedence is higher
                            || (top_el->mOp->prec == it->mOp->prec &&
                                top_el->mOp->assoc ==
                                    OperatorAssociativity::
                                        left)  // if current in stack operator
                                               // == current operator, and they
    ...
    static std::string initializeOperationCombination() {
        std::string res{};
        for (const Operator& el : mOperators) res += el.strRepr;
        return res;
    }
    ...
    static bool tokenize(std::string_view expr) {
        // Initializing string that is a combination of strReprs of all
        // operators: "+-/*" Need to initialize it only once -> that is why it
        // is static local
        static std::string operatorsCombined{initializeOperationCombination()};

        for (auto it_a{expr.begin()}, end{expr.end()}; it_a != end;) {
            DEBUG("index = "
                  << it_a - expr.begin() << ", starts_with_operator = "
            ...
            if (int operatorId =
                    starts_with_operator(std::string_view(it_a, end - it_a));
                operatorId != -1) {
                mTokens.emplace_back(TokenType::operatorr,
                                     &mOperators[operatorId], nullptr);
                it_a += mOperators[operatorId].strRepr.length();

            } else {
                // end_of_number_index = (end of number - begining_of_number)
                size_t number_end_index =
                    expr.find_first_of(operatorsCombined, it_a - expr.begin());

                DEBUG("br2) number_end_index = " << number_end_index << '\n');

                size_t number_str_length =
                    (number_end_index == std::string::npos)
                        ? end - it_a
                        : number_end_index - (it_a - expr.begin());

                DEBUG("br2) number_str_length = " << number_str_length << '\n');

                std::string_view strRepr{it_a, number_str_length};

                BI intRepr{strRepr};
                mTokens.emplace_back(
                    TokenType::integer, nullptr,
                    std::make_shared<Integer>(strRepr, intRepr));
    ...
    static int starts_with_operator(std::string_view str) {
        for (int i = 0, end = mOperators.size(); i < end; ++i)
            if (str.rfind(mOperators[i].strRepr, 0) == 0) return i;
        return -1;
    }
};
...
std::vector<typename Calculator<BI>::Operator> Calculator<BI>::mOperators{
    Calculator<BI>::Operator{
        OperatorId::leftBracket,
        "(",
        OperatorPrecedence::none,
        OperatorAssociativity::none},
    Calculator<BI>::Operator{
        OperatorId::rightBracket,
        ")",
        OperatorPrecedence::none,
        OperatorAssociativity::none},

    Calculator<BI>::Operator{
        OperatorId::plus, "+", OperatorPrecedence::low,
        OperatorAssociativity::left, &BI::operator+ },
    Calculator<BI>::Operator{
        OperatorId::minus, "-", OperatorPrecedence::low,
        OperatorAssociativity::left, &BI::operator- },
    Calculator<BI>::Operator{
        OperatorId::multiply, "*", OperatorPrecedence::medium,
        OperatorAssociativity::left, &BI::operator* },
    Calculator<BI>::Operator{
        OperatorId::divide, "/", OperatorPrecedence::medium,
        OperatorAssociativity::left, &BI::operator/ },
    ...
    Calculator<BI>::Operator{
        OperatorId::exponentiate, "^", OperatorPrecedence::high,
        OperatorAssociativity::right, &BI::pow} };
