...
class TestUtils {
 public:
    ...
    explicit TestUtils(unsigned int _seed = 0)
        : seed{_seed},
          mt{seed},
          distrib{storage_type_t{0}, storage_type_t{0} - 1} {
        // if no seed provided -> make seed random
        if (seed == 0) mt.seed(rd());
    }

    // need OperandWrapper to abstract away conversions to strings of operands
    // of different types because for BigInt it was BigInt::get_as_string()
    ...
    class OperandPtrWrapper {
     ...
     public:
        ...
        std::string get_as_string(bool param1 = false,
                                  bool param2 = false) const {
    ...
    class OperandPtrWrapper<BigInt<T, size>> {
     ...
     public:
        ...
        const BigInt<T, size>& operator*() const { return *m_ptr; }
        const BigInt<T, size>* operator->() const { return m_ptr; }
    };
    ...
    static std::string parse_response(const std::string& target) {
        static const std::regex r{
            "<div id=\"bignumber\" style=\"padding: 10px; font-family: "
            "monospace; "
            "border: solid black 2px;\">[\\s]+(.*)<\\/div>"};
        std::smatch match{};
        ...
        for (size_t entry_index{};
             (entry_index = str_match.find(new_line_delimiter, entry_index)) !=
             std::string::npos;)
    ...
    unsigned int generate_number() { return distrib(mt); }

    template <typename OP1_TYPE, typename OP2_TYPE = OP1_TYPE>
    static void check_binary_operation(
        const OP1_TYPE& operand1,  // OP1_TYPE - is always BigInt<A, x> -> no
                                   // need to wrap it
        const OperandPtrWrapper<OP2_TYPE>&
            operand2_ptr,  // OP2_TYPE - BigInt<A, x> or int
        const std::string& str_repr_of_operator,
        const std::string& str_urlencoded_repr_of_operator,
        std::function<OP1_TYPE(const OP1_TYPE*, const OP2_TYPE&)>
            operation_func) {
        std::string request_str_command =
            "curl -s -X POST -d 'eqn=" + operand1.get_as_string(true, false) +
            str_urlencoded_repr_of_operator +
            operand2_ptr.get_as_string(true, false) +
            "&submit=Calculate&base=hex' "
            "https://defuse.ca/big-number-calculator.htm";

        auto my_response{
            operation_func(&operand1, *operand2_ptr).get_as_string()};

        auto html_dump{exec(request_str_command.c_str())};
        ...
        if (auto res = (my_response == parsed_response)) {
            ...
            std::string error_info{
                std::to_string(res) + " <- " + operand1.get_as_string(true) +
                str_repr_of_operator + operand2_ptr.get_as_string(true) + "\n" +
                "my: " + my_response + "\n" + "his:" + parsed_response + "\n"};
            std::string error_log_command{"echo \"" + error_info +
                                          "\" >> log.txt"};
    ...
    static std::string url_encode_expr_string(std::string input) {
        ...
        static std::string operatorsStrCombined{
            ...
            []() -> std::string {
                std::string res{};
                for (const auto& el : operators_url_encoded) res += el.first;
                return res;
            }()};
        ...
        for (size_t index{0};;) {
            ...
            input.replace(index, 1,
                          operators_url_encoded[std::string{input[index]}]);
    ...
    static void format_response(std::string* parsed_response) {
        ...
        // we need this, because defuse.ca can sometimes return us numbers
        // larger than we can handle
        auto resp_len = parsed_response->length();
        if (resp_len > BigInt<_T, _bitlength>::max_hex_digits) {
            parsed_response->erase(
                0, resp_len - BigInt<_T, _bitlength>::max_hex_digits);
            parsed_response->erase(0, parsed_response->find_first_not_of('0'));
        }

        if (*parsed_response == "") *parsed_response = "0";
    }

    // [?] workaround for: "Sorry, we can't calculate numbers THAT big!" when
    // using ^ since ^ works differently in defuse.ca, I can check ^ separately
    // using python pow(x, y, p):
    //     1) would have to divide expression, so that ^ will be calculated on
    //     python
    ...
    static void check_compound_expression(std::string expression) {
        std::string request_str_command =
            "curl -s -X POST -d 'eqn=" + url_encode_expr_string(expression) +
            "&submit=Calculate&base=hex' "
            "https://defuse.ca/big-number-calculator.htm";
        ...
        if (auto res = (my_response == parsed_response)) {
            ...
            std::string error_info{expression + '\n' + "my: " + my_response +
                                   '\n' + "his: " + parsed_response + '\n'};
            std::string error_log_command{"echo \"" + error_info +
                                          "\" >> log.txt"};
            std::cout << std::boolalpha << error_info;
...
std::unordered_map<std::string, std::string>
    TestUtils<_T, _bitlength>::operators_url_encoded{{{"(", "%28"},
                                                      {")", "%29"},
                                                      {"+", "%2B"},
                                                      {"-", "-"},
                                                      {"*", "*"},
