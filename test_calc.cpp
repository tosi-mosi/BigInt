...
int main() {
    auto test_cases = std::vector<std::string>{
        {"0x2284/"
         "0x5+0x1235+0x12555+(0x2*(0x3+0x3))+0x12315125*(0x1+0x3^0x5541)*"
         "0x51871^"
         "0x151513^413",  // good
         "0x1231515^0x11+((0x458727592876987fa52+0x411235)*0x516)^0x3",
         "0x15299285292859285+0x15299285292859285*0x15299285292859285-"
         "0x15299285292859285+0x1231515+0x1231515*0x5",
         "0x1+0x1"}};

    for (auto &el : test_cases) test_utils_t::check_compound_expression(el);
}
