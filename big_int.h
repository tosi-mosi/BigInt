...
#include <memory>
#include <sstream>
#include <string>  // cppling suggests to add this for std::string
#include <string_view>
#include <utility>  // cpplint suggests to add this for std::move
...
// [+][-][+][solved] BUG: fasf -> it reads without throwing exception - not what
// I want
...
class BigInt {
 public:
    ...
    constexpr static int max_hex_digits =
        (max_bit_length / 4);  // 1 hex digit = 4 bits
    constexpr static int StorageNum =
        (max_bit_length) / (sizeof(STORAGE_TYPE) * 8);
    ...
    constexpr static STORAGE_TYPE ms_half_mask{max_el_value
                                               << (storage_el_bit_length / 2)};
    constexpr static STORAGE_TYPE ls_half_mask{~ms_half_mask};

 private:
    // not making member std::array directly, because big array obj would take
    // up a lot of stack space,
    //  since it's just a wrapper over T[N].
    std::unique_ptr<std::array<STORAGE_TYPE, StorageNum>> m_parr;

    enum class ComparisonOption { smaller, equal, greater };
 ...
 public:
    ...
    explicit BigInt(std::string_view str_repr) : BigInt{} {
        // check if template parameter max_bit_length complies to requirements
        static_assert(max_bit_length % (sizeof(STORAGE_TYPE) * 8) == 0 &&
                          max_bit_length > 0,
                      "max_bit_length should be a multiple of "
                      "sizeof(STORAGE_TYPE) and > 0");

        // if prepended with 0x, then remove 0x
        if (str_repr[0] == '0' && str_repr[1] == 'x') str_repr.remove_prefix(2);
        ...
        auto max_hex_digits_in_general =
            hex_digs_in_one_storage_element * StorageNum;
        ...
        for (int i{str_payload_len}; i > 0;
             i -= hex_digs_in_one_storage_element) {
            ...
            // I need tellg = -1, because when after extracting stream has smth
            // left
            // -> sets failbit -> tellg = -1
            bool b_extracted = static_cast<bool>(
                ss >> (*m_parr)[(str_payload_len - i) /
                                hex_digs_in_one_storage_element]);
            bool b_characters_left_in_ss = ss.tellg() != -1;

            if (!b_extracted || b_characters_left_in_ss) {
                DEBUG("b_extracted = " << b_extracted
                                       << ", b_chars_lef_in_ss = "
                                       << b_characters_left_in_ss << '\n');
                throw std::runtime_error(std::string{"BigInt invalid input "} +
                                         tmp_str);
            }
    ...
    //   -> less performant than move, but still allows copying, which is cool I
    //   guess
    ...
    BigInt operator>>(int operand) const {
        ...
        for (int i{0}; i < StorageNum - shift_storage_elements_val; ++i) {
            ...
            auto mask2 = ~mask1;
            (*result.m_parr)[i] |=
                (((*m_parr)[i + shift_storage_elements_val] & mask1)) >>
                shift_bits_val;
            if (i + shift_storage_elements_val < StorageNum - 1)
                (*result.m_parr)[i] |=
                    ((*m_parr)[i + shift_storage_elements_val + 1] & mask2)
                    << (storage_el_bit_length - shift_bits_val);
        }
    ...
    BigInt operator+(const BigInt &rhs) const {
        ...
        for (auto i{0}; i < BigInt::StorageNum; ++i) {
            ...
            if ((i != BigInt::StorageNum - 1)) {
                // (*rhs.m_parr)[i] + (*result.m_parr)[i]) may wrap around,
                //   so check if it wraps here, if it wraps -> then carry = 1
                //   automatically
                if (((*rhs.m_parr)[i] == max_el_value &&
                     (*result.m_parr)[i] == 1) ||
                    ((max_el_value - (*m_parr)[i] <
                      (*rhs.m_parr)[i] + (*result.m_parr)[i])))
                    (*result.m_parr)[i + 1] += 1;
            }
    ...
    BigInt operator-(const BigInt &rhs) const {
        ...
        for (auto i{0}; i < BigInt::StorageNum; ++i) {
            ...
            // check_for_wraparound, if wraparound happened -> carry = 1
            if (i != StorageNum - 1)
                if (((*rhs.m_parr)[i] == max_el_value && carry == 1) ||
    ...
    BigInt operator*(const BigInt &rhs) const {
        ...
        int half_bitsize_of_storage_type =
            storage_el_bit_length / 2;  // (( x/8 ) * 2) = x/4 if 8|x
        ...
        for (auto i{0}; i < StorageNum * 2; ++i) {
            ...
            for (int j{0}; j < StorageNum * 2; ++j) {
                ...
                if (i + j < StorageNum * 2) {
                    tmp =
                        (i % 2 == 0 ? ((*m_parr)[i / 2] & ls_half_mask)
                                    : ((*m_parr)[i / 2] >>
                                       half_bitsize_of_storage_type)) *
                        (j % 2 == 0 ? ((*rhs.m_parr)[j / 2] & ls_half_mask)
                                    : ((*rhs.m_parr)[j / 2] >>
                                       half_bitsize_of_storage_type)) +
                        ((i + j) % 2 == 0
                             ? ((*result.m_parr)[(i + j) / 2] & ls_half_mask)
                             : ((*result.m_parr)[(i + j) / 2] >>
                                half_bitsize_of_storage_type)) +
                        carry;
                    ...
                    if ((i + j) % 2 == 0)
                        (*result.m_parr)[(i + j) / 2] =
                            ((*result.m_parr)[(i + j) / 2] & ms_half_mask) |
                            (tmp & ls_half_mask);
                    else
                        (*result.m_parr)[(i + j) / 2] =
                            ((*result.m_parr)[(i + j) / 2] & ls_half_mask) |
                            (tmp << half_bitsize_of_storage_type);
 ...
 private:
    ...
    void set_bit(int bit_index) {
        if (bit_index >= max_bit_length) return;
        (*m_parr)[bit_index / storage_el_bit_length] |=
            STORAGE_TYPE{1} << (bit_index % storage_el_bit_length);
    }
 ...
 public:
    ...
    BigInt operator/(const BigInt &rhs) const { return division(rhs)[0]; }

    BigInt operator%(const BigInt &rhs) const { return division(rhs)[1]; }

    bool get_bit(unsigned int bit_index) const {
        return ((*m_parr)[bit_index / storage_el_bit_length] &
                (STORAGE_TYPE{1} << (bit_index % storage_el_bit_length))) != 0;
    }
 ...
 public:
    ...
    // since defuse.sa has no >,=,< ... -> utilize "-" operation -> minus sign
    // as indicator
    bool operator>(const BigInt &rhs) const {
        return comparison_helper(rhs) == ComparisonOption::greater ? true
                                                                   : false;
    }
    ...
    bool operator<(const BigInt &rhs) const {
        return comparison_helper(rhs) == ComparisonOption::smaller ? true
                                                                   : false;
    }

    bool operator>=(const BigInt &rhs) const {
        auto tmp{comparison_helper(rhs)};
        return tmp == ComparisonOption::greater ||
                       tmp == ComparisonOption::equal
                   ? true
                   : false;
    }
    ...
    bool operator<=(const BigInt &rhs) const {
        auto tmp{comparison_helper(rhs)};
        return tmp == ComparisonOption::smaller ||
                       tmp == ComparisonOption::equal
                   ? true
                   : false;
    }

    std::string get_as_string(bool prepend_with_0x = false,
                              bool upper_case = false) const {
        STORAGE_TYPE zero_storage_el{0};

        std::stringstream ss{};

        if (prepend_with_0x) ss << "0x";

        if (upper_case) ss << std::uppercase;
    ...
    template <typename _U, int __bit_length>
